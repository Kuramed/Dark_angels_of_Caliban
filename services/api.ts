...
export const apiFetch = async (endpoint: string, options?: RequestInit) => {
    ...
    if (endpoint.includes('localhost:4000')) {
        cleanEndpoint = endpoint.split('localhost:4000/')[1] || endpoint.split('localhost:4000')[1] || '';
    }
    ...
    if (method === 'GET') {
        if (id) return db[table].find((item: any) => item[idKey] == id || item.id == id);
        if (query) {
            const [key, val] = query.split('=');
            return db[table].filter((item: any) => String(item[key]) === String(val));
        }
        return db[table];
    }

    if (method === 'POST') {
        const newItem = options?.body ? JSON.parse(options.body as string) : {};
    ...
    if (method === 'PUT') {
        const updatedItem = options?.body ? JSON.parse(options.body as string) : {};
