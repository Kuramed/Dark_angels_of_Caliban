    ...
    return (
        <div className="container mt-4">
            <div className="d-flex justify-content-between mb-3">
                ...
                <Link to="/trilhas/novo" className="btn btn-primary">Nova Trilha</Link>
            </div>
                ...
                <tbody>
                    {trilhas.map(trilha => (
                        <tr key={trilha.id_trilha}>
                            <td><strong>{trilha.titulo}</strong></td>
                            <td className="small text-muted">{trilha.descricao}</td>
                            <td>
                                <Link to={`/trilhas/editar/${trilha.id_trilha}`} className="btn btn-sm btn-warning me-2">Editar</Link>
                                <button className="btn btn-sm btn-danger" onClick={() => TrilhaService.excluir(trilha.id_trilha!).then(() => window.location.reload())}>
                                    Eliminar
                                </button>
                            </td>
                        </tr>
