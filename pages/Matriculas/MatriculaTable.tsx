    ...
    return (
        <div className="container mt-4">
            <div className="d-flex justify-content-between mb-3">
                ...
                <Link to="/matriculas/novo" className="btn btn-primary">Nova Matrícula</Link>
            </div>
                ...
                <tbody>
                    {matriculas.map(matricula => (
                        <tr key={matricula.id_matricula}>
                            ...
                            <td>
                                <span className={`badge ${matricula.status === 'Ativa' ? 'bg-success' : 'bg-secondary'}`}>
                                    {matricula.status}
                                </span>
                            </td>
                            <td>
                                <Link to={`/matriculas/editar/${matricula.id_matricula}`} className="btn btn-sm btn-warning">Editar</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
