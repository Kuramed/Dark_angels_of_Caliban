        ...
        <div className="container mt-4">
            ...
            <form onSubmit={handleSubmit} className="card p-4 mt-3 shadow-sm">
                ...
                <div className="mb-3">
                    ...
                    <input type="number" className="form-control" value={formData.nota}
                        onChange={e => setFormData({ ...formData, nota: Number(e.target.value) })} min="1" max="5" required />
                </div>
                <div className="mb-3">
                    <label className="form-label">Comentário</label>
                    <textarea className="form-control" rows={3} value={formData.comentario}
                        onChange={e => setFormData({ ...formData, comentario: e.target.value })} />
                </div>
                <div className="d-flex gap-2">
                    <button type="submit" className="btn btn-success">Guardar</button>
                    <button type="button" className="btn btn-secondary" onClick={() => navigate('/avaliacoes')}>Cancelar</button>
                </div>
            </form>
        </div>
    );
