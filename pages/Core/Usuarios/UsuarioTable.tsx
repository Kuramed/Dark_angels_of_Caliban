...
export function UsuarioTable() {
    ...
    // Função separada para podermos recarregar a tabela facilmente após excluir
    const carregarUsuarios = () => {
        UsuarioService.listarTodos().then(setUsuarios);
    };

    const handleExcluir = async (usuario: any) => {
        // Pega o id verdadeiro (seja id_usuario do nosso modelo ou id gerado pelo json-server)
        ...
        // Uma caixinha de segurança para evitar exclusões acidentais
        const confirmar = window.confirm(`Tem a certeza que deseja excluir o utilizador ${usuario.nomeCompleto}?`);
    ...
    return (
        <div className="container mt-4">
            <div className="d-flex justify-content-between mb-3">
                <h2>Utilizadores</h2>
                <Link to="/usuarios/novo" className="btn btn-primary">Novo Utilizador</Link>
            </div>
            ...
            <table className="table table-striped border shadow-sm">
                ...
                <tbody>
                    {usuarios.map((usuario: any) => {
                        // Garante que a chave da linha seja única usando o ID disponível
                        ...
                        return (
                            <tr key={chaveId}>
                                ...
                                <td>{usuario.tipo}</td>
                                <td>
                                    <Link to={`/usuarios/editar/${chaveId}`} className="btn btn-sm btn-warning me-2">Editar</Link>
                                    <button 
